...
int run(char *videoPath)
{
    ...
    while (true)
    {
        ...
        detect_service.predict(img, bboxes, 0.01, 0.45);

        // yolo output format to bytetrack input format, and filter bbox by class id
        std::vector<TrackObject> objects;
        for (long unsigned int j = 0; j < bboxes.size(); j++)
        {
            cv::Rect r = bboxes[j].rect;
            float conf = (float)bboxes[j].prob;
            int class_id = (int)bboxes[j].label;

            if (isTrackingClass(class_id))
            {
                cv::Rect_<float> rect((float)r.x, (float)r.y, (float)r.width, (float)r.height);
                TrackObject obj{rect, class_id, conf};
...
int main()
{
    run("E:/DeepLearning/track_utils/videos/scene_1.mp4");
}
